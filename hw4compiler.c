...
void addSymbolToTable(int kind, int *tx, int *dx, int lev, int numVarScope){
    ...
    //Proc
        ...
        symbolTable[numsymbols].level = lev;
        symbolTable[numsymbols].addr = 3*line;
        //printf("LOOK A ME %d\n", symbolTable[numsymbols].addr);
    }
    numsymbols++;
...
void PROC_DECLARATION(int lev, int *tx, int *dx) {
    ...
    while (token == procsym) {
        ...
        //increase numProc and declare the procedure
        numProc++;
        addSymbolToTable(3, tx, dx, lev, numProc); // add the procedure to the symbol table
        //Store address and line to be used later
        ...
        symbolTable[addrTemp].addr = lineTemp*3; // Set the address of the procedure in the symbol table
...
void STATEMENT(int lev, int *tx){
    ...
    if(token == identsym){
        ...
        symLevel = getSymbolLevel(token, lev);
        if(symIdx == -1){
        ...
        emit("STO", lev-symLevel, symbolTable[symIdx].addr);
    ...
    //if else
        ...
        int symLevel = getSymbolLevel(token, lev);
        if (symIdx == -1) {
            printErrorMessage("undeclared");
            return;
        }
...
void FACTOR(int lev) {
    ...
    if (token == identsym) {
        ...
        if (symbolTable[symIdx].kind == 1) { // const
            ...
            emit("LOD", lev-symLevel, symbolTable[symIdx].addr);
        }
        nextToken();
    } else if (token == numbersym) {
        emit("LIT", 0, number);
...
void printErrorMessage(const char* errorTerm) {
    ...
    } else if (strcmp(errorTerm, "call") == 0) {
        printf("Error: 'call' must be followed by a procedure name.\n");
    } else if (strcmp(errorTerm, "else") == 0) {
        printf("Error: 'else' must be preceded by 'then'.\n");
    } else {
